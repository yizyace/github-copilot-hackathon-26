...
describe('serializeSoulStack', () => {
  it('emits the .soul/ tree: manifest, three md files, one journal file per memory', () => {
    const files = serializeSoulStack(patternBuddyStack)
    ...
    expect(paths.filter((p) => p.startsWith('.soul/journal/')).length).toBe(patternBuddyStack.journal.length)
  })
  ...
  it('writes valid manifest fields and the journal frontmatter + body skeleton', () => {
    ...
    expect(manifest).toContain('name: pattern-buddy')
